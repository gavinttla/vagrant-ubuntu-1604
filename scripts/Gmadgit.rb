class Gmadgit
    def Gmadgit.configure(config, settings)
        # Set The VM Provider
        ...
        # Configure The Box
        config.vm.define settings["name"] ||= "Gmadgit-7"
        #config.vm.box = settings["box"] ||= "laravel/Gmadgit"
		config.vm.box = settings["box"] ||= "ubuntu/xenial64"
        config.vm.box_version = settings["version"] ||= ">= 4.0.0"
        config.vm.hostname = settings["hostname"] ||= "Gmadgit"
        ...
        config.vm.provider "virtualbox" do |vb|
            vb.name = settings["name"] ||= "Gmadgit-7"
        ...
        ["vmware_fusion", "vmware_workstation"].each do |vmware|
            config.vm.provider vmware do |v|
                v.vmx["displayName"] = settings["name"] ||= "Gmadgit-7"
        ...
        config.vm.provider "parallels" do |v|
            v.name = settings["name"] ||= "Gmadgit-7"
        ...
        if settings.include? 'keys'
            if settings["keys"].to_s.length == 0
                puts "Check your Gmadgit.yaml file, you have no private key(s) specified."
                exit
            end
            settings["keys"].each do |key|
                if File.exists? File.expand_path(key)
                    ...
                    puts "Check your Gmadgit.yaml file, the path to your private key does not exist."
                    exit
                end
            end
        end
        ...
        if settings.include? 'folders'
            settings["folders"].each do |folder|
                if File.exists? File.expand_path(folder["map"])
                    ...
                    config.vm.provision "shell" do |s|
                        s.inline = ">&2 echo \"Unable to mount one of your folders. Please check your folders in Gmadgit.yaml\""
                    end
                end
            end
        end
        ...
        if settings.has_key?("variables")
            settings["variables"].each do |var|
                ...
                config.vm.provision "shell" do |s|
                    s.inline = "echo \"\n# Set Gmadgit Environment Variable\nexport $1=$2\" >> /home/vagrant/.profile"
                    s.args = [var["key"], var["value"]]
                end
            end
